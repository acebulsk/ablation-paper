...
f(u) = \begin{cases}
    0,& \text{if } u < u_{th} \\
    u_{mid} \cdot a \cdot e^{b\cdot u_{mid}} \cdot e^{c\cdot T_a},              & \text{if }  u \ge u_{th}
\end{cases}
$$ {#eq-wind-unld}

where $u_{mid}$ is the wind speed at mid canopy height, $a$ is a constant of `r mod_coef_wind[['a']]` (m^-1^), $b$ is a constant of `r mod_coef_wind[['b']]` (m^-1^), $c$ is a constant of `r mod_coef_wind[['c']]` (m^-1^) and $u_{th}$ is a threshold wind speed below which $f(u)$ is equal to zero.
...
where rates of $q_{drip}$ was simulated from the updated CRHM canopy snowmelt routine, $q_{unld}^{wind}$ was simulated using @eq-wind-unld, and $q_{subl}^{veg}$ was simulated using the CRHM canopy snow sublimation routine.

![The relationship between the canopy snow unloading rate, attributed to the melting of canopy snow measured by weighed tree lysimetry, and the rate of simulated canopy snowmelt. The dots represent the unloading and snowmelt rates observed at 15-minute intervals. The red line indicates a line of best fit using @eq-melt-unld-ode.](figs/results/modelled_melt_unloading_w_obs.png){#fig-mod-melt-unld}
