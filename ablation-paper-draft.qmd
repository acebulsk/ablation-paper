...
$q_{unld}^{melt}$ was calculated as a residual from the observed ablation from the weighed tree:

$$
q_{unld}^{melt} = \Delta L - q_{drip} - q_{unld}^{wind}
 - q_{sub}^{veg}
$${#q_unld_melt_mass_bal}

where rates of $q_{drip}$ was simulated from the updated CRHM canopy snowmelt routine, $q_{unld}^{wind}$ was simulated using @eq-wind-unld, and $q_{subl}^{veg}$ was simulated using the CRHM canopy snow sublimation routine. The rate of canopy snow unloading due to the melt of canopy snow was found to occur at `r q_unld_melt_ratio` times the $q_{drip}$ rate which is a function of canopy snow load and ice-bulb temperature. The data used to find this ratio is from periods where snow is in the canopy after snowfall events and the canopy is loaded with more than 2 kg m^-2^ of snow and for observations that had more than 0.1 kg m^-2^ ablated from the canopy over a 15 minute timestep.
...
An updated canopy snow energy balance routine was implemented to better represent the canopy snowmelt and sublimation rates. This approach is based on the following one-layer energy balance equation that treats the snow separate from vegetation elements:
...
where $Q_{melt}(L)$ (W m^-2^) is the rate of energy available to melt canopy snow, $Q_{sw}$ and $Q_{lw}$ (W m^-2^) are the net shortwave and longwave radiation heat fluxes to the canopy snow, $Q_p$ (W m^-2^), is the advective energy rate, and $Q_{l}$ and $Q_{h}$ (W m^-2^), are the turbulent fluxes of latent heat and sensible heat (positive towards canopy snow), respectively, $C_p^{ice}$ (J kg^-1^ K^-1^) is the specific heat capacity of ice, $\frac{\Delta T_{vs}}{\Delta t}$ (K s^-1^) is the rate of change of the canopy snow temperature. 
...
$$
where $\downarrow Q_{sw}$ is the downwelling shortwave radiation (W m^-2^), $\alpha_s$ is the albedo of snow intercepted in the canopy (taken as 0.8), $\tau_{50}^{veg}$ is the transmittance of radiation through half of the vegetation elements [Eq. 7 from @Pomeroy2009 with half the LAI]. $Q_{sw}$ thus approximates the solar radiation incident to the mid height point of the canopy which is thought to represent an average value across differing heights within the canopy. Upwelling shortwave radiation reflected off of the surface snowpack is considered negligible contribution to the snow intercepted in the canopy as it is primarily blocked by vegetation elements underlying the canopy snow. 
...
$$
where $Q_{lw}^{atm}$ is downwelling longwave radiation from the atmosphere with the @Sicart2006 emissivity calculation for the influence of clouds, $Q_{lw}^{veg}$ is longwave radiation upwelling from the vegetation elements underlying the snow intercepted in the canopy which is assumed to be in equilibrum with the air temperature plus the increase in vegetation temperature from shortwave irradiance extinguished in the canopy approxiamted from Equation 4 in @Pomeroy2009, $Q_{lw}^{veg2veg}$ is longwave radiation emitted from the underside of the vegetation elements but not absorbed by the subcanopy snowpack and reflected back to the canopy, and $Q_{lw}^{vs}$ is the outgoing longwave radiation from the top and bottom of the canopy snow layer calculated as:
...
### The influence of sublimation and snowmelt on unloading

TODO maybe do similar probability plot here. also need to determine if air temp relationship is still valid for non meling snow. 
...
The canopy snow unloading to melt ratio was found to be to be linearly proportional to the canopy snow load as shown in @fig-mod-melt-unld. This was represented as the equation:

<!-- consider adding Lres to melt unloading function , see eq. 29 in the review paper -->
$$
q_{unld}^{melt} = X_{mu} \cdot q_{drip}(L)
$${#eq-melt-unld-ode}

where $X_{mu}$ is the ratio of unloading to canopy snowmelt. @eq-melt-unld-ode is similar to the equation presented by @Andreadis2009 but differs in that instead of incorporating a minimum threshold of $L$ for $q_{unld}^{melt}$ to occur, $X_mu$ was found to increase with increasing $L$ as:

$$
X_{mu} = m \cdot L
$$

where $m$ is a fitting parameter found to be (~ 0.1, *R^2^* = 0.95) through an ordinary least squares regression for the data shown in @fig-unld-melt-ratio which were filtered to periods of snowmelt and no precipitation.

Similarly the ratio of canopy snow unloading to canopy snow sublimation was also found to be proportional to the canopy snow load [@fig-unld-subl-ratio]. This was represented by the equation:

$$
q_{unld}^{subl} = X_{su} \cdot q_{subl}^{veg}
$$

where $X_{su}$ is the ratio of unloading to canopy snow sublimation. $X_su$ was found to increase with increasing $L$ as:

$$
X_{mu} = s \cdot L
$$

where s is a fitting parameter found to be (~ 0.2, *R^2^* = 0.78) through an ordinary least squares regression for the data shown in @fig-unld-subl-ratio which were filtered to periods of sublimation and no precipitation.

![The relationship between the canopy snow unloading to snowmelt ratio for varying canopy snow load bins. The dots represent the cumulative observed unloading divided by cumulative simulated snowmelt for various bins of canopy snow load. The red line indicates a line of best fit through ordinary least squares regression.](figs/results/modelled_melt_unloading_ratio_vs_snow_load_bin.png){#fig-unld-melt-ratio}
