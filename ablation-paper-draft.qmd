...
The rate of canopy snow unloading due to wind was quantified by the subcanopy lysimeters during intervals without canopy snowmelt. By excluding periods influenced by melt, the snow captured by the lysimeters is attributed primarily to wind-driven unloading. While unloading caused by snow metamorphism or thermally induced branch bending may also contribute, the measured unloading is interpreted to be predominantly from wind-induced unloading over these intervals. The subcanopy lysimeters had a relatively low accumulation of unloaded snow over the 15-min intervals, leading to high relative instrument error of measured unloading. To improve instrument accuracy, while maintain consistency of the unloading measurements with wind speed, the 15-min interval measurements of unloading were aggregated over differing bins of wind speed. Wind speed bins that had accumulated unloaded snow less than 0.1 mm were removed which resulted in a mean instrument error of +/- 2% for the remaining bins.

A mass balance approach was incorporated to determine the unloading rate resulting from canopy snow melt ($q_{unld}^{melt}$, mm s^-1^). Since direct measurements of canopy snow unloading are challenging to obtain independently from canopy snowmelt drainage [@Storck2002], the following mass balance was incorporated to determine $q_{unld}^{melt}$ as a residual:
...
where $\frac{dL}{dt}$ is the change in canopy snow load over time, (mm s^-1^), $q_{sf}$ is the snowfall rate (mm s^-1^), $q_{tf}$ (mm s^-1^) is the throughfall rate (mm s^-1^), $q_{ros}$ (mm s^-1^) is the rate of rainfall falling on snow intercepted in the canopy, $q_{unld}$ is the canopy snow unloading rate (mm s^-1^), $q_{drip}$ is the canopy snow drip rate due to canopy snowmelt (mm s^-1^), $q_{wind}^{veg}$ is the wind transport rate in or out of the control volume (mm s^-1^), and $q_{sub}^{veg}$ is the intercepted snow sublimation rate (mm s^-1^). Figure 1 in @Cebulski2025 presents a visual representation of this mass balance. 

Assuming that canopy snow unloading $q_{unld}$ originates from two sources: $q_{unld}^{melt}$ and wind-induced unloading ($q_{unld}^{wind}$):
...
Canopy snow unloading was estimated using the method outlined by @Ellis2010, @Roesch2001, and @Andreadis2009 which are described in detail in @Cebulski2025. New canopy snow unloading parameterisations were also developed based on the measurements presented in this study and were implemented in CRHM for assessment.
...
The influence of snow load, canopy snowmelt and wind speed on the canopy snow unloading rate, measured by the subcanopy lysimeters, was observed to be statistically significant (*p* < 0.05, *R^2^* = `r lm_multi_reg_tbl$R2[1] |> round(2)`) from the multiple linear regression (@tbl-q-unld-bins). The mean unloading rate was also observed to increase with increasing canopy snowmelt and wind speed in @fig-q-unld-all-bins. The effect of air temperature and canopy snow sublimation was found to be insignificant (*p* > 0.05) and resulted in nearly the same *R^2^* value when adding either or both predictors to the model with canopy snow load, canopy snowmelt, and wind speed (@tbl-q-unld-bins). Air temperature and canopy snow sublimation were positively associated with canopy snow unloading, though for sublimation this relationship was limited to positive values (@fig-q-unld-all-bins). Across all bins, the canopy snow unloading rate was observed to be higher for increasing canopy snow loads (@fig-q-unld-all-bins). Most of the variability in canopy snow unloading was described by the canopy snowmelt rate (*p* < 0.05, *R^2^* = `r melt_only_r2 |> round(2)`) increasing slightly when adding canopy snow load to (*p* < 0.05, *R^2^* = `r melt_tree_only_r2 |> round(2)`). However, canopy snow unloading exhibited a much higher degree of variance (0–2 mm/hr) across the canopy snowmelt and sublimation bins compared to the air temperature and wind speed bins, which only varied between 0–0.5 mm/hr. This greater variability may explain the higher explanatory power of the canopy snowmelt predictor.

```{r tbl-q-unld-wind, echo=FALSE}
#| label: tbl-q-unld-bins
#| tbl-cap: "Summary of multiple linear regression results evaluating all combinations of predictor variables for canopy snow unloading. Columns L to Ta show the coefficient estimate for each respective term, and the significance of each term is shown in brackets. Significance codes: '*' = p < 0.05; ns = not significant (p > 0.05). The models are ranked by their corresponding AIC value."
#| tbl-colwidths: [11, 13, 13, 14, 12, 14, 14, 8, 8]


lm_multi_reg_tbl |> 
  select(`Model Name`, intercept, L, u, q_melt, q_subl, T_a, R2, AIC) |> 
...
Canopy snow unloading measurements from the subcanopy lysimeters, filtered to include intervals without canopy snowmelt, followed an exponential relationship with wind speed (*p* < 0.05, *R^2^* = `r q_unld_wind_r2`). The wind-induced unloading rate was observed to be higher for bins with higher canopy snow load across most bins (@fig-q-unld-wind). The *R^2^* of this relationship is much higher than the amount of variance explained by wind speed when including intervals with melting snow, as suggested by the difference in *R^2^* between the M1 and M5 models of ~0.03 (@tbl-q-unld-bins). The increased liquid water content during the canopy snowmelt process may provide some resistance to wind induced unloading. Moreover, increased canopy snow unloading during low wind speeds, resulting from high melt rates could also contribute to the lower explanatory power of wind speed in the M1 model in @tbl-q-unld-bins.
...
#| tbl-cap: "Summary of the performance and regression coefficiencts for the relationship between canopy snow unloading with wind speed as shown in @fig-q-unld-wind."

wind_err_tbl |> knitr::kable()
```

An exponential equation to represent the canopy snow unloading rate due to wind, $q_{unld}^{wind}$, as shown in @fig-q-unld-wind is:
...
<!-- much higher snow loads observed in @Cebulski2025b and thus the exposure coef in @Pomeroy1998 was adjusted -->
