...
The canopy snow unloading rate due to melt and wind can be represented by:

$$
q_{unld} = L \cdot \kappa_{unld}
$${#eq-unl-ode}

where:

$$
\kappa_{unld} = f(T_a) + f(u)
$$
...
To derrive an analytical solution for $q_unld$ we start with the analytical equation to compute the canopy snow load for the next time step, $L^{t+\Delta t}$ due to unloading:

$$
L^{t+\Delta t} = L^{t} \cdot exp(-\kappa_{unld} \Delta t)
$${eq-qunl-analytical}

where $L_t$ is the canopy snow load for the current time step. 

Since change in canopy snow load due to unloading over a discrete time interval, $\Delta L$ aka ($q_{unld} \cdot \Delta t$) is calculated as:
...
$$
\Delta L = L^{t} - L^{t} \cdot exp(-\kappa_{unld} \Delta t)
$$

Factor out $L^{t}$:

$$
\Delta L = L^{t} \cdot \left[1 - exp(-\kappa_{unld} \Delta t)\right].
$$
