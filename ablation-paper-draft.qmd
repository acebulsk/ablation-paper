...
Canopy snow unloading due to melt of snow intercepted in the canopy is shown in @fig-mod-melt-unld to be linearly proportional to the canopy snow melt rate, $q_{drip}$. This was represented as the equation:

<!-- consider adding Lres to melt unloading function , see eq. 29 in the review paper -->
$$
q_{unld}^{melt} = X_u \cdot q_{drip}(L)
$${#eq-melt-unld-ode}

where $X_u$ is the ratio of mass release to canopy snowmelt. @eq-melt-unld-ode is similar to the equation presented by @Andreadis2009 but differs in that it does not incorporate a minimum threshold of $L$ for $q_{unld}^{melt}$ to occur. A linear model to represent the relationship shown in @fig-mod-melt-unld was derived using @eq-melt-unld-ode and an ordinary least squares regression fit. $q_{unld}^{wind}$ calculated as a from the observed ablation from the weighed tree:
...
![The relationship between the canopy snow unloading rate, attributed to the melting of canopy snow measured by weighed tree lysimetry, and the rate of simulated canopy snowmelt. The dots represent the unloading and snowmelt rates observed at 15-minute intervals. The red line indicates a line of best fit using @eq-melt-unld-ode.](figs/results/modelled_melt_unloading_w_obs_binned.png){#fig-mod-melt-unld}

```{r}
melt_err_tbl <- readRDS('data/modelled_melt_unloading_error_table.rds') 
...
$$
\Delta L = L^{t} - \left[L^{t} \cdot exp(-f(u) \Delta t)\right] + \left[X_u \cdot q_{drip} \cdot \Delta t\right]
$$

Factor out $L^{t}$ from the unloading term:

$$
\Delta L = L^{t} \cdot \left[1 - exp(-f(u) \Delta t)\right]+ \left[X_u \cdot q_{drip} \cdot \Delta t\right]
$$

### Model Results 

![](figs/crhm-analysis/subl_drip_unld/select_post_agu_events/compare_baseline_updated_psp_w_new_wind_unld_w_temp_par/obs_mod_canopy_snow_load.png){#fig-obs-mod-w-tree}


```{r}
event_smry <- left_join(
  event_met,
  obs_mod_w_tree_err_tbl
) |> left_join(event_ablation_frac)
