...
snow unloading relationships with snow load and wind shear stress. All
...
alter the quantity (Essery et al., 2003; Mazzotti et al., 2021; Pomeroy
\& Gray, 1995; Rojas-Heredia et al., 2024; Sanmiguel-Vallelado et al.,
2020) and timing (Ellis et al., 2010; Safa et al., 2021) of snow that
reaches the ground. Snow falls in forests over half of the Northern
Hemisphere (Kim et al., 2017) and over 23\% of the landmass globally
(Deschamps-Berger et al., 2025), making forest-snow interactions crucial
to understand for informed ecological, land management, and water
resource decision-making. Needleleaf canopies are particularly effective
at intercepting snowfall (Cebulski \& Pomeroy, 2025b; Hedstrom \&
Pomeroy, 1998; Pomeroy \& Schmidt, 1993; Storck et al., 2002), where
intercepted snow is subject to increased energy inputs relative to the
subcanopy snowpacks, leading to increased rates of melt, unloading,
and/or sublimation (Parviainen \& Pomeroy, 2000; Pomeroy et al., 1998b;
Roesch et al., 2001; Storck et al., 2002). The partitioning of snow to
the atmosphere via sublimation (Parviainen \& Pomeroy, 2000; Pomeroy et
al., 1998b) or to the ground through unloading and meltwater drip
(Lumbrazo et al., 2022; Roesch et al., 2001; Storck et al., 2002) is
highly sensitive to meteorological conditions and forest structure
contributing to substantial variability in subcanopy snowpacks across
regions and snowfall events. Coastal humid environments typically
...
Pomeroy, 2025a, 2025b). This separation allows for individual
parameterisations for distinct processes, improving both process
representation and the modular design of contemporary models, thereby
supporting broader applicability across diverse environments and model
structures (Clark et al., 2015; Pomeroy et al., 2022). In addition,
Lundquist et al. (2021) and Cebulski \& Pomeroy (2025b) demonstrated
that canopy snow processes can be more accurately represented without
the concept of a maximum canopy snow load, which is included in initial
accumulation parameterisations (Andreadis et al., 2009; Hedstrom \&
Pomeroy, 1998). Existing ablation routines were integrated in canopy
snow accumulation parameterisations, likely as a result of some ablation
included in measurements of interception efficiency (Cebulski \&
Pomeroy, 2025a). For example, Hedstrom \& Pomeroy (1998) found that
interception efficiency declined as the canopy is loaded with snow,
while Storck et al. (2002) found a constant interception efficiency of
0.6 and is also typically combined with a maximum canopy snow load.
Consequently, both the Hedstrom \& Pomeroy (1998) and Storck et al.
(2002) parameterisations have significantly lower interception
efficiencies---prior to canopy snow ablation---compared to the
observations by Cebulski \& Pomeroy (2025b). Staines \& Pomeroy (2023)
and Cebulski \& Pomeroy (2025b) show that interception efficiency is
best predicted by canopy density alone---consistent with some rainfall
interception parameterisations (e.g., Valante et al., 1997; Zhong et
al., 2022)---and that snow load has a small influence on canopy density,
challenging the Hedstrom \& Pomeroy (1998) and Storck et al. (2002)
initial snow interception theories. Together these studies (Cebulski \&
Pomeroy, 2025a, 2025b; Lundquist et al., 2021) emphasize the need to
revisit canopy snow ablation parameterisations.
...
2012), canopy snowmelt rate (Storck et al., 2002), and wind speed (see
Supporting Information for example relationships). Each of these factors
were also found to be dependent on canopy snow load and time. While
Hedstrom \& Pomeroy (1998) did not make direct observations of canopy
snow unloading, they proposed a parameterisation based on canopy snow
load and time. In addition to the empirical observations by the
afformentioned studies, physical reasoning also supports the inclusion
of these processes. For example, melt promotes unloading through loss of
structural integrity, particle bond weakening, and lubrication of
intercepted snow. Wind drag promotes unloading through shear stress
applied to intercepted snow, wind erosion through direct entrainment in
the atmosphere of intercepted snow, and branch movement. Branch
elasticity increases with temperature (Schmidt \& Pomeroy, 1990) which
can increase the likelihood of unloading due to increasing branch angle
under a snow load and decreased stiffness to resist swaying in a
turbulent wind.

Investigation of canopy snow unloading relationships is challenged by
the number of physical processes possible for removing snow from the
canopy. Given the correlation of predictor variables when considered
over all events (dry snow and melt-driven unloading), and differing
physical drivers of unloding during these events and potential diverging
relationships with predictor variables the distribution of variables
over melt and non-melt events were investigated if they are
statistically from differing distributions to justify investigation and
relationships for these two event types separately. Moreover, some
predictor variables such as air temperature may have a positive
association during canopy snowmelt periods due to increased branch
flexibility and lubrication and structural degredation of canopy snow
clumps during melt and versus negative association over dry-snow
unloading due to the association of cohesion/adhesion with air
temperature/melt. Condensation during canopy snowmelt events may
increase structural integrety of canopy snowclumps due to rime ice
/frost build up but may have an opposing affect during dry snow
unloading events as sublimation degrades the structure of canopy snow
clumps. In addition to differing relationships many of the possible
predictors are correlation for example, melt is correlated with wind
speed through increase in turbulent energy transfer. Wind speed and
sublimation are correlated non-linearly although this passes the
multicolinerty test which tests for linear correlation.
...
Ridge Station (FRS) 2.0 km to the northwest of FT station (50.8° N,
115.2° W). This windy exposed site was selected to reduce snow
...
(2009) \\
R01 & Roesch et al., (2001) & \(f(\text{snow load}, \text{wind speed})\)
& \(f(\text{snow load, air temp.})\) \\
...
remaining bins. Air temperature and wind speed were measured at the FT
...
sublimation simulated using CRHM with the CP25 model as described in the
previous section.
...
processes unrelated to wind. Both the wind speed and shear stress
relationships violated the homoscedasticity assumption (Breusch-Pagan
test \emph{p} \textless{} 0.05; see Supporting Information section),
with residual variance increasing for higher unloading rates. To address
this issue, generalized least squares regression was applied, which
accounts for differing residual variances by appropriately weighting
observations.
...
were not used concurrently in any model evaluation. The Supporting
Information provides full correlation matrices and complete formulations
of the additional models tested.

Although the wind speed relationship had reduced RMSE, the improved
accuracy based on higher \emph{R\textsuperscript{2}} and \emph{d}
statistics and lower mean bias, coupled with the better physical
representation of kinetic energy, indicate that shear stress is the
preferred predictor of dry snow unloading (Table~\ref{tbl-dry-unld}).
Accordingly, the dry unloading rate, \(q_{unld}^{dry}\), was represented
as a linear function of shear stress:
...
midpoint of each bin (0--2, 2--6, 6--20 kg m\textsuperscript{-2}).}
...
for GLS \\
HS & Pass & Pass & Pass & Pass (n.s.) & Pass (n.s.) & Pass & Pass \\
...
\emph{R\textsuperscript{2}} = 0.37, RMSE = 0.39 mm
hr\textsuperscript{-1}) suggesting wind driven unloading is also
prevalent when intercepted snow is melting. The superior performance of
...
the energy balance. In contrast, the temperature-based predictors
exhibited lower performance (\emph{p} \textless{} 0.05,
\emph{R\textsuperscript{2}} \(\le\) 0.1, RMSE \(\approx\) 0.5 mm
hr\textsuperscript{-1}). Certain predictor pairs---namely shear stress
with wind speed and air temperature with ice-bulb
temperature---displayed strong linear and nonlinear associations
(Pearson or Spearman coefficients \textgreater{} 0.7) and were therefore
not included together in any model. Canopy snowmelt rate, air
temperature, and ice-bulb temperature exhibited moderate correlation
(Pearson or Spearman coefficients \textgreater{} 0.5) but were excluded
as their known physical interdependence (full correlation statistics are
...
rate (\(q_{melt}\)), with the remaining variables defined in
Table~\ref{tbl-dry-unld}.}
...
\(p\)(b) & n.s. & NA & NA & NA \\
HS & Pass (n.s.) & Pass (n.s.) & Pass (n.s.) & Pass (n.s.) \\
Norm & Pass (n.s.) & Pass (n.s.) & Pass (n.s.) & Pass (n.s.) \\
...
using the ordinary least squares regression shown in
Figure~\ref{fig-unld-melt-ratio}. This resulted in a statistically
significant relationship (\emph{p} \textless{} 0.05,
\emph{R\textsuperscript{2}} = 0.73, RMSE = 0.79 (-)). The relationship
exhibited heteroscedasticity, with residual variance increasing at
higher unloading-to-melt ratios (Breusch-Pagan test \emph{p} \textless{}
0.05; see Supporting Information) and was accounted for by using a
generalised least squares regression. See the Supporting Information
section for additional statistical details. Additional observations of
canopy snowmelt from the subcanopy rain gauges were also used to
estimate \(R\) (Figure~\ref{fig-unld-melt-ratio}). The number of usable
observations was limited to three events (out of the 5 warm \& humid
events) due to freeze-thaw events that seized up the tipping bucket
mechanisms in the rain gauges, however, these measurements are still
useful in providing some validation of the CRHM canopy snowmelt/drip
calculations (Figure~\ref{fig-cml-tb}). The CRHM-estimated cumulative
drip was higher than the subcanopy rain gauges for two out of the three
melt events. Differences in the timing and magnitude of the observed and
simulated values were expected due to both instrument uncertainties in
the rain gauges from freezing of rain gauges and thawing of snow in the
...
canopy snow ablation events. The seventeen ablation events had air
temperatures ranging from -30.5°C to +6.9°C and wind speeds from calm to
5.3 m s\textsuperscript{-1} (Figure~\ref{fig-event-met-boxplot}). This
range of meteorological conditions is particularly wide and spans
conditions typical of the cold boreal to temperate maritime needleleaf
forests. Events were classified as cold \& dry, cold \& humid, warm \&
dry, and warm \& humid based on the median event air temperature (above
or below 0°C) and relative humidity (above or below 65\%)
...
at FT station. Note: the rectangle vertical extent represents the
...
respectively; due to their differing liquid water storage capacities
...
mm, exceeding the threshold predicted by all models.
...
showed the poorest performance, likely due to the absence of key
processes such as wind-induced unloading (E10 and SA09) and
energy-balance-based melt (E10). The R01 model showed the second-highest
performance, which may reflect the inclusion of both melt and
wind-induced unloading processes, although its reliance on an
air-temperature index melt formulation likely explains its reduced
accuracy relative to CP25.
...
to be constant at 0.4. The measurement difficulties noted by Storck et
al. (2002) limited their estimate of this ratio to a single mid-December
...
of unloading during melt events (Table~\ref{tbl-melt-unld}) supports the
application of Equation~\ref{eq-q-unld-tau} during canopy snowmelt
periods. However, further research is required to evaluate the
efficiency of wind-driven unloading as snow transitions from dry to
melting.
...
Supporting Information. The higher \emph{R\textsuperscript{2}} found for
shear stress compared to wind speed for predicting unloading---when
excluding melt events---is likely due to the physical relationship
between shear stress force and kinetic energy transfer to the canopy,
wind transport from the canopy, and movement of branches in the canopy
induced by drag (shear) forces. The differing relationship presented
here, compared to the R01 model, may be attributed to the development of
...
2025a), and may have contributed to R01 overestimating wind-driven
unloading (Figure~\ref{fig-obs-mod-w-tree}). Conversely, the subcanopy
lysimeter measurements employed here provided a more direct
quantification of canopy snow unloading rates. Additional factors that
are not considered in the new parameterisations include wind erosion,
branch movement, structural degradation, bond weakening, increased
elasticity of branches (associated with species and age), snow density,
and liquid water content. These factors would all directly have
implications for the canopy snow ablation routines presented in this
study. For example, compression of branches would increase the slope
angle supporting intercepted snow increasing the likelihood of
unloading. The addition of liquid water content in the canopy snow due
to phase change can increase (Pomeroy \& Gray, 1995) or decrease
, Figure~\ref{fig-unld-melt-ratio} cohesion and adhesion of snow clumps
within the canopy which may also affect the dry-snow unloading rate,
although this effect was not observed in this study. Sublimation of
intercepted snow reduces its structural integrity (MacDonald, 2010), and
equi-temperature metamorphism may increase density and cohesion both
with differing associations with canopy snow unloading rates and the
exposure of intercepted snow to turbulent energy exchanges. Although
these factors did not emerge as dominant processes in this study, the
parameterisations presented here should be tested in environments where
these processes are expected.
...
processes which had limited transferability across differing events (R01
and E10). Although the SA09 model---originally developed in a relatively
warm maritime climate with limited wind influence (Storck et al.,
2002)---performed similarly well to CP25 during most of the
...
over all events (Figure~\ref{fig-partitioning}). Moreover, the constant
...
mm, whereas the CP25 model more accurately predicted ablation over this
event. The low liquid water retention capacity and increased solid snow
unloading for snow loads in SA09 also contributed to an underestimation
of canopy loads during the end of most melt events
(Figure~\ref{fig-obs-mod-w-tree}).
...
canopy, which failed to represent the energy availability over these
events. In contrast, all four models performed similarly during warm \&
dry events, where air temperatures were closer to 0°C, and lower
humidities increased the impact of cooling from canopy snow sublimation.
The superior performance of the energy balance-based canopy snowmelt
models (CP25 and SA09) during warm \& humid events likely reflects their
...
to R01 aside from one event with high wind speeds (\textgreater5 m
...
applicability. The infrequent occurrence of wind-transport from the
canopy in our observations may account for underestimation of ablation
during one strongly wind-dominated events by CP25 (2023-02-24,
...
reduced performance over these events. Moreover, a comprehensive
representation of both dry-snow and melt-induced unloading was shown to
be crucial in correctly representing snowfall partitioning at this study
site. This is expected to also be important in other regions which are
...
with climate warming. Amongst all canopy snow ablation processes, dry
snow unloading introduced the most uncertainty. Although the revised
model performed best for this dataset, further validation is required
across a wider range of climates and forest structures. Since unloading,
melt, and sublimation are competitive ablation processes, they strongly
influence whether snow is returned to the atmosphere or reaches the
ground.
...
models {[}e.g., Clark et al. (2015); Ellis et al. (2010); this study{]}
via the latent heat flux parameterisation, it is usually treated as
additions to the canopy snow reservoir where the unloading rate is
typically overestimated (Lumbrazo et al., 2022). Due to the cold
continental location of this study, canopy rime and/or frost deposition
was not observed, and thus the unloading rates presented here would also
likely overestimate unloading rates for ice. Thus, the transferability
of these results may be limited in maritime climates where deposition of
rime and frost is more likely due to the higher humidity and warmer
temperatures. The addition of a canopy ice reservoir could help better
represent this processes, however further research is needed to
investigate how ablation rates differ in these environments.
...
and ablation processes (Cebulski \& Pomeroy, 2025a, 2025b), will improve
process representations, these routines still need to be evaluated
together against additional field observations. Incorporating the
...
to insufficient process-level validation. This study represents the
first development and validation of an unloading model addressing both
energy balance-based melt and dry snow unloading processes together.
Novel parameterisations for dry snow and melt-induced unloading were
introduced, with key differences from previously established approaches.
Canopy snow load and shear stress were found to be the best predictors
of dry snow unloading (\emph{R\textsuperscript{2}} = 0.58). The canopy
melt rate exerted the strongest control on snow unloading during melt
events, consistent with one existing model, and show improved
performance compared to temperature index melt routines. A new finding
was that the ratio of unloading to canopy snowmelt increased with canopy
snow load (\emph{R\textsuperscript{2}} = 0.73). In addition to this
empirical evidence, physical processes such as structural degradation,
snow particle bond weakening, lubrication of wet canopy snow during
melt, and the shear force exerted on canopy snow by wind further support
...
dry unloading events, though performance was improved when incorporating
a site-specific shear stress-based parameterisation. Partitioning of
intercepted snow disposition between the ground and atmosphere varied
most amongst cold events, where neglecting the dry snow unloading
process resulted in considerable overestimates of canopy snow
sublimation losses. Observations were collected over a wide range of
meteorological conditions typical of cold boreal to temperate maritime
climates, with air temperatures from -30.5°C to +6.7°C and wind speeds
from calm to 5.3 m s\textsuperscript{-1}. Some processes, such as
...
the energy balance. Although improved performance was found for the
updated canopy snow ablation model compared to existing methods, across
a wide range of meteorological conditions, additional testing across
various climate and forest compositions is required to assess model
transferability.
