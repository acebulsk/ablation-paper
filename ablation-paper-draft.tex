...
Lundquist et al., 2021; Roesch et al., 2001). Conversely, the colder and
drier winters in continental climates typical of the boreal forests can
induce substantial canopy sublimation losses (e.g., 25---45\% of annual
...
shown in \textbf{?@fig-unl-ex-wind} for (Bartlett \& Verseghy, 2015;
...
(\textbf{?@fig-unl-ex-wind}). While Hedstrom \& Pomeroy (1998) did not
...
(Ellis et al., 2010; Floyd, 2012) (\textbf{?@fig-unl-ex-temp}).
...
canopy snowmelt and sublimation were calculated using CRHM. The canopy
snow load corresponds to the mean of each bin (0--2, 2--6, 6--20 kg
m\textsuperscript{-2}).}
...
(left) and Equation~\ref{eq-q-unld-wind} (right). The canopy snow load
corresponds to the mean of each bin (0--2, 2--6, 6--20 kg
m\textsuperscript{-2}).}
...
Figure~\ref{fig-unld-melt-ratio}. Equation~\ref{eq-q-unld-melt} using
these fitted parameters resulted in a statistically significant
relationship (\emph{p} \textless{} 0.05, \emph{R\textsuperscript{2}} =
0.73). Additional observations of canopy snowmelt from the subcanopy
rain gauges were also used to estimate \(R\)
...
humidity, and wind speed over each of the seventeen select ablation
...
in \textbf{?@fig-unl-ex-wind}. The higher \emph{R\textsuperscript{2}}
...
presented here, compared to the R01 model (\textbf{?@fig-unl-ex-wind}),
...
Verseghy, 2015; Roesch et al., 2001). This approach would have included
both unloading and sublimation processes, in addition to greater
measurement uncertainties (Cebulski \& Pomeroy, 2025a). Conversely, the
subcanopy lysimeter measurements employed here provided a more direct
