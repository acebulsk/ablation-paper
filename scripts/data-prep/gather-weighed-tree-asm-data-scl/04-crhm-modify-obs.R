...
ccs <- unique(combined_tree_events$tree_cal_cc)

# write out separate obs file for each canopy type
for (cc in ccs) {
  
  combined_tree_events_sel_cc <- combined_tree_events |> 
    filter(tree_cal_cc == cc) |> 
    select(-tree_cal_cc)
  
  ft_obs_irtc_tree <- ft_obs_irtc |> 
    left_join(combined_tree_events_sel_cc) |> # change this if want cold events too
    mutate(obs_snow_load = ifelse(is.na(obs_snow_load), 9999, obs_snow_load))
  
  CRHMr::writeObsFile(ft_obs_irtc_tree,
                      paste0('crhm/obs/ffr_crhm_modelling_obs_w_asm_data_cc', cc, '.obs'),
