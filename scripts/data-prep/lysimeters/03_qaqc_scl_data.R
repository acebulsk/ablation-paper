...
scl_df <- scl_df_kg_m2_raw |> 
  inner_join(canopy_snow_long |> select(datetime, event_id, bad_troughs)) |> 
  # filter(event_id %in% c('2021-12-14', '2022-01-09')) |>
  rename(mixed = trough_1,
         sparse = trough_2,
         closed = trough_3) |> 
  pivot_longer(tree:TB4_mm) |> 
  mutate(row_flag = mapply(grepl, name, bad_troughs)) |>   # remove troughs where flagged
  filter(!row_flag)
...
pc_jump_correct$split_var <- paste0(pc_jump_correct$event_id, '_', pc_jump_correct$name)
df_split <- split(pc_jump_correct, pc_jump_correct$split_var)
...
saveRDS(scl_out, 'data/clean-data/treefort_scl_qaqc.rds')
...
loads <- scl_df_kg_m2_raw |>
  ...
  select(datetime, event_id, tree, trough_1, trough_2, trough_3) |>
  pivot_longer(tree:trough_3) |>
  rbind(new_df_to_compare)

# stroms_zeroed <- loads |>
#   group_by(event_id, name) |>
#   mutate(zeroed = value - first(value)) |> 
#   filter(year(datetime) %in% c(2021))
# 
# ggplot(stroms_zeroed |> filter(is.na(event_id) == F, name != 'tree'), aes(datetime, y = zeroed, colour = name, group = name)) +
#   geom_line() +
