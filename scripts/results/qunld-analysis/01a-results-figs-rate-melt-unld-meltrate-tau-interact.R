...
model_lm <- lm(q_unl_avg ~ tree_labs:canopy_snowmelt_labs:tau_labs - 1, data = unld_met_smry)
summary(model_lm)
...
coef_tbl <- tibble(
  ...
  Value = c(
    coefs_df$`tree_labs:canopy_snowmelt_labs:tau_labs_Estimate`,
    coefs_df$`tree_labs:canopy_snowmelt_labs:tau_labs_p_value`,
    NA,
    NA
  )
)
...
eqn <- tibble(
  Metric = 'Equation',
  Value  = "$q_{unld}^{melt} = L \\cdot q_{melt}^{veg} \\cdot \\tau_{mid} \\cdot a$"
)
...
saveRDS(long_tbl,
        'data/results/modelled_melt_unloading_melt_w_tau_interact_error_table.rds')
