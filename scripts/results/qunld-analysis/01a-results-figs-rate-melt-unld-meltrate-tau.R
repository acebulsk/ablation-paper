...
unld_met_smry <- obs_mod_met_melt |> 
  ...
  group_by(tree_labs, canopy_snowmelt_dml_labs, tau_labs) |> 
...
ggplot(unld_met_smry, 
       aes(x = canopy_snowmelt_dml_labs, y = q_unl_avg, colour = tau_labs, shape = as.character(round(tree_labs)))) + 
...
model_lm <- lm(q_unl_avg ~ tree_labs:canopy_snowmelt_dml_labs + tau_labs - 1, data = unld_met_smry)
summary(model_lm)
...
coef_tbl <- tibble(
  ...
  Value = c(
    ...
    coefs_df$tau_labs_Estimate,
    coefs_df$tau_labs_p_value
  )
)
...
eqn <- tibble(
  Metric = 'Equation',
  Value  = "$q_{unld}^{melt} = L \\cdot \\frac{q_{melt}}{L} \\cdot a + \\tau_{mid} \\cdot b$"
)
...
saveRDS(long_tbl,
        'data/results/modelled_melt_unloading_melt_w_tau_error_table.rds')
