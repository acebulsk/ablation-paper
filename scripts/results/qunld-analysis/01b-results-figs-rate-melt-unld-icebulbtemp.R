...
unld_met_smry <- obs_mod_met_melt |> 
  ...
  group_by(tree_labs, ice_temp_labs) |> 
...
ggplot(unld_met_smry, 
       aes(x = ice_temp_labs, y = q_unl_avg, colour = as.character(round(tree_labs)))) + 
...
model_lm <- lm(q_unl_avg ~ tree_labs:ice_temp_labs - 1, data = unld_met_smry)
summary(model_lm)
...
ex_temp_labs <- unld_met_smry$ice_temp_labs |> unique()
ex_tree_labs <- c(1, 4)
ex_df <- expand.grid(ice_temp_labs = ex_temp_labs, tree_labs = ex_tree_labs)
# tau_ex_df$new_predicted_y_nls <- predict(model_nls, newdata = tau_ex_df)
ex_df$new_predicted_y <- predict(model_lm, newdata = ex_df)

# stopifnot(all(unld_met_smry$ice_temp_labs %in% ex_temp_labs))

## PLOT MODEL ----
plot_df <- ex_df |> left_join(unld_met_smry) |>
  pivot_longer(ice_temp_labs, names_to = 'x_var_name', values_to = 'x_var_value')
...
coef_tbl <- tibble(
  ...
  Value = c(
    coefs_df$`tree_labs:ice_temp_labs_Estimate`,
    coefs_df$`tree_labs:ice_temp_labs_p_value`,
    NA,
    NA
  )
)
...
eqn <- tibble(
  Metric = 'Equation',
  Value  = "$q_{unld}^{melt} = L \\cdot T_i \\cdot a$"
)
...
saveRDS(long_tbl,
        'data/results/modelled_melt_unloading_ice_temp_error_table.rds')
