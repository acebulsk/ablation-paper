...
warning("Moderate linear and non-linear correlation between shear stress and sublimation found below. 
Coupled with physical correlation between these two variables should not be used together in a regression.")

## Check multicollinerity 

ggplot(met_unld_no_melt_cold, aes(tau, q_subl_veg)) + 
  geom_point()

ggplot(met_unld_no_melt_cold, aes(tau_labs, subl_labs)) + 
  geom_point()

# check collinerarity 

predictors <- met_unld_no_melt_cold[, c("tree_mm", "tau", "q_subl_veg")] # moderate linear correlation between tau and subl
...
library(car)
lm_col <- lm(q_unl ~ tree_mm + tau + q_subl_veg, data = met_unld_no_melt_cold)
vif(lm_col)

# test nonlinear correlation
x <- met_unld_no_melt_cold$tau
y <- met_unld_no_melt_cold$q_subl_veg
cor(x, y, method = "spearman") # 0.5 non linear correlation 
...
met_unld_no_melt_tau_smry <- met_unld_no_melt_cold |> 
  ...
  group_by(tau_labs, tree_labs, subl_labs) |> 
...
model_lm_no_subl <- update(model_lm, . ~ . - subl_labs) # temp is insignificant 

AIC(model_lm, model_lm_no_subl)
anova(model_lm, model_lm_no_subl, test = "LRT")
...
model_wls <- lm(q_unl_avg ~ tree_labs:tau_labs + subl_labs - 1,
...
# gls_exp <- gls(q_unl_avg ~ tree_labs:tau_labs + subl_labs - 1,
#                data = met_unld_no_melt_tau_smry,
#                method = "REML",
...
AIC(model_lm, gls_power, gls_ident)     # lower AIC from gls power ... 
...
tau_ex_df <- expand.grid(tau_labs = ex_tau_labs, tree_labs = ex_tree_labs, subl_labs = ex_subl_labs)
...
# ggsave(
#   'figs/results/modelled_tau_subl_unloading_w_obs.png',
#   width = 6,
#   height = 4,
#   device = png
# )
...
saveRDS(long_tbl,
        'data/results/modelled_tau_subl_unloading_error_table.rds')
