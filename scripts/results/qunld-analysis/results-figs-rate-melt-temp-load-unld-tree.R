...
# due to mass unloading of snow as it melts with air temp and canopy load.
...
obs_mod_fltr <- obs_mod |> 
  filter(canopy_snowmelt.1 > 0.015,
         est_q_unld_melt >= 0,
         hru_t.1 > 0
  ) |> 
...
ggplot(obs_mod_fltr) +
  geom_point(
    aes(hru_t.1, canopy_snowmelt.1,
        colour =  Snow_load.1),
    alpha = 1
  ) +scale_color_viridis_c()

# Fit model
lm_model_sm_t_L_int <- lm(est_q_unld_melt ~ 0 + hru_t.1:canopy_snowmelt.1 + tree_mm,
                      data = obs_mod_fltr)

obs_mod_fltr$pred_sm_t_L_int <- predict(lm_model_sm_t_L_int)

lm_model_sm_t_L <- lm(est_q_unld_melt ~ 0 + hru_t.1 +canopy_snowmelt.1 + tree_mm,
                      data = obs_mod_fltr)

obs_mod_fltr$pred_sm_t_L <- predict(lm_model_sm_t_L)

lm_model_sm_L <- lm(est_q_unld_melt ~ 0 + canopy_snowmelt.1 + tree_mm, data = obs_mod_fltr)

obs_mod_fltr$pred_sm_L <- predict(lm_model_sm_L)

lm_model_t_L <- lm(est_q_unld_melt ~ 0 + hru_t.1 + tree_mm, data = obs_mod_fltr)

obs_mod_fltr$pred_t_L <- predict(lm_model_t_L)

lm_model_sm <- lm(est_q_unld_melt ~ 0 + canopy_snowmelt.1, data = obs_mod_fltr)

obs_mod_fltr$pred_sm <- predict(lm_model_sm)

lm_model_t <- lm(est_q_unld_melt ~ 0 + hru_t.1, data = obs_mod_fltr)

obs_mod_fltr$pred_t <- predict(lm_model_t)
...
q_unld_melt_lm <- lm(q_unl_avg ~ avg_w_tree:canopy_snowmelt_labs - 1, data = obs_mod_fltr_avg)
obs_mod_fltr_avg$mod_q_unld_melt <- obs_mod_fltr_avg$canopy_snowmelt_labs * obs_mod_fltr_avg$avg_w_tree * 0.46

# Create new data for predictions
new_data <- expand.grid(
  avg_w_tree = unique(df$avg_w_tree),
  canopy_snowmelt_labs = seq(min(df$canopy_snowmelt_labs), max(df$canopy_snowmelt_labs), length.out = 100)
)

# Predict values
new_data$q_unl_pred <- predict(q_unld_melt_lm, newdata = new_data)
...
ggplot(obs_mod_fltr_avg) + 
  ...
  geom_abline(intercept = 0, slope = sqrt(20)*2, color = "red",    # Model line
              linetype = "solid", size = 0.5) +
  geom_abline(intercept = 0, slope = sqrt(3)*2, color = "blue",    # Model line
              linetype = "solid", size = 0.5) 
...
# Create the plot
ggplot(obs_mod_fltr, aes(x = canopy_snowmelt.1, y = est_q_unld_melt, colour = avg_w_tree)) +
  geom_point(size = 2) +                      # Scatter plot of the data
  geom_abline(intercept = 0, slope = sqrt(20), color = "red",    # Model line
              linetype = "solid", size = 0.5) +
  geom_abline(intercept = 0, slope = sqrt(3), color = "blue",    # Model line
              linetype = "solid", size = 0.5) +
  labs(
