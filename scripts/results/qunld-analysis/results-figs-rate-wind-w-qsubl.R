...
# sublimation effect which hypothesized to increase efficency of unloading as
# snow is mechanically removed

# no real difference between sublimation rates

# FIRST CHECK TEMP AND WIND ARE INDEPENDENT ----

check <- left_join(q_unld_scl, met_binned, by = c('datetime', 'name'))
ggplot(check, aes(wind_labs, vs_labs)) + 
  geom_point(alpha = 0.1)
cor.test(check$wind_labs, check$vs_labs)
...
met_unld_all_winds_cold <- 
  ...
  mutate(temp_group = case_when(
    vs_labs < 0.0005 ~ 'Less Subl.',
    TRUE  ~ 'More Subl.'
  )) |> group_by(temp_group) |> 
  mutate(avg_subl = mean(vs_labs))

met_unld_all_winds_cold |> 
  group_by(`Canopy Load (mm)`, avg_subl) |> 
  summarise(avg_w_tree = mean(tree_mm))

met_unld_all_winds_cold_smry <- met_unld_all_winds_cold |> 
  group_by(wind_labs, `Canopy Load (mm)`, avg_w_tree, avg_subl) |> 
...
ggplot(met_unld_all_winds_cold_smry, 
       aes(x = wind_labs, y = q_unl_avg, colour = factor(round(avg_subl, 5)))) + 
...
model_nls <- nls(q_unl_avg/(60*60) ~ wind_labs * a * avg_w_tree * exp(b * wind_labs) * exp(-c * (avg_subl + 20)), 
...
met_unld_all_winds_cold_smry <- met_unld_all_winds_cold_smry |> 
  mutate(
    model_nls_pred = wind_labs * nls_coefs[[1]] * avg_w_tree * exp(nls_coefs[[2]]* wind_labs) * exp(-nls_coefs[[3]] * (avg_subl + 20))
  )
# Create ggplot
ggplot(met_unld_all_winds_cold_smry, aes(x = wind_labs, y = model_nls_pred, colour = avg_subl)) +
  geom_point() +
...
model_nls <- nls(q_unl_avg ~ wind_labs * a * avg_w_tree * exp(b * wind_labs) * exp(-c * (avg_subl + 20)), 
                 data = met_unld_all_winds_cold_smry, 
                 start = list(a = a_lm, b = b_lm, c = 0.1))
...
ex_avg_subl <- c(-21, -10)
wind_ex_df <- expand.grid(wind_labs = ex_wind_labs, avg_w_tree = ex_avg_w_tree, avg_subl = ex_avg_subl)
new_predicted_y <- predict(model_nls, newdata = wind_ex_df)

## PLOT MODEL ----

ggplot(wind_ex_df) +
  geom_line(aes(wind_labs, new_predicted_y, colour = factor(round(avg_subl)))) +
  geom_errorbar(data = met_unld_all_winds_cold_smry,
                aes(
                  ...
                  ymin = sd_low,
                  width = 0.15,
                  colour = factor(round(avg_subl))
                )) +
  geom_point(data = met_unld_all_winds_cold_smry,
             aes(wind_labs, q_unl_avg, colour = factor(round(avg_subl))),
             size = 2) +
